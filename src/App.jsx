...
const BlurText = ({
  ...
  return (
    <p ref={ref} aria-label={text} className={`inline-flex flex-wrap justify-center ${className}`} style={style}>
      {segments.map((segment, i) => (
        <span
          ...
          style={{
            ...
            filter: (inView || reducedMotion) ? "blur(0px)" : "blur(10px)",
            opacity: (inView || reducedMotion) ? 1 : 0,
            transform: (inView || reducedMotion)
              ? "translateY(0)"
...
function PortfolioHero() {
  ...
  const handleThemeChange = (newTheme) => {
    ...
    }).catch(() => { /* The selected theme still applies if the animation is interrupted. */ });
  };
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <div
      id="top" className="min-h-screen transition-colors overflow-hidden bg-gray-50 dark:bg-black text-gray-900 dark:text-white"
    >
      { }
      <header className={`fixed top-0 left-0 right-0 z-50 px-6 py-6 w-full transition-all duration-300 ${isScrolled ? "bg-white/70 dark:bg-black/70 backdrop-blur-md py-4 shadow-sm" : "bg-transparent py-6"}`}>
        <nav aria-label="Main navigation" className="flex items-center justify-between max-w-screen-2xl mx-auto font-mono">
          ...
          <div className="hidden md:flex items-center gap-12 text-[14px] text-gray-700 dark:text-gray-300">
            ...
            <a href="#integrations" className="hover:text-black dark:hover:text-white transition-colors">Toolkit</a>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <details className="mobile-menu md:hidden">
              <summary>Menu <ChevronDown size={14} /></summary>
              <div className="mobile-menu-links" onClick={(event) => {
                if (event.target.closest('a')) event.currentTarget.closest('details').open = false;
              }}>
                <a href="#about">About</a><a href="#projects">Projects</a>
                <a href="#integrations">Toolkit</a><a href="#contact">Contact</a>
              </div>
            </details>
            <a href="https://github.com/Ironankit525" target="_blank" rel="noreferrer" className="bg-slate-900 text-white dark:bg-white dark:text-slate-900 p-2 rounded-full inline-flex items-center justify-center transition-all duration-200 hover:bg-black dark:hover:bg-gray-200 hover:scale-105" aria-label="GitHub">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path>
                <path d="M9 18c-4.51 2-5-2-7-2"></path>
              </svg>
            </a>
            <a href="https://www.linkedin.com/in/ankit-kumar525/" target="_blank" rel="noreferrer" className="bg-slate-900 text-white dark:bg-white dark:text-slate-900 p-2 rounded-full inline-flex items-center justify-center transition-all duration-200 hover:bg-black dark:hover:bg-gray-200 hover:scale-105" aria-label="LinkedIn">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      { }
      <section className="hero-stage relative z-0 flex flex-col" aria-label="Introduction">
        <h1 className="sr-only">Ankit Kumar — AI-focused developer</h1>
        { }
        ...
        />
        { }
        <div className="hero-art absolute left-1/2 -translate-x-1/2 -translate-y-1/2 w-full px-4">
          <div className="relative text-center flex flex-col items-center">
            <div className="z-0 relative">
              <BlurText
                ...
                animateBy="letters"
                direction="top"
                className="hero-name font-bold leading-[0.75] tracking-tighter uppercase justify-center whitespace-nowrap"
        ...
        { }
        <div className="hero-copy absolute left-1/2 -translate-x-1/2 w-full px-6">
          <div className="flex flex-col justify-center items-center">
            <BlurText
              ...
              animateBy="words"
              direction="top"
              className="text-[15px] sm:text-[18px] md:text-[20px] lg:text-[22px] text-center transition-colors duration-300 text-neutral-600 dark:text-neutral-300"
            ...
            <BlurText
              ...
              animateBy="words"
              direction="top"
              className="mt-4 text-[13px] sm:text-[15px] md:text-[17px] lg:text-[19px] text-center transition-colors duration-300 text-neutral-600 dark:text-neutral-400 max-w-2xl mx-auto"
              style={{ fontFamily: "'Antic', sans-serif" }}
            />
          </div>
        </div>
        ...
        >
          <ChevronDown className="w-6 h-6 md:w-8 md:h-8 text-neutral-600 dark:text-neutral-300 transition-colors duration-300" />
        </a>
      </section>
    </div>
  );
...
import { projectForRoute } from "./data/projectModel";
...
export default function App() {
  ...
  const isProjectRoute = currentRoute.startsWith("#/project/");
  return (
    <>
      ...
      <main className="studio">
        <AboutSection />
        <FeaturedProjectsSection items={projects} />
        <IntegrationSection />
        <ContactSection />
        <footer className="studio-footer">
          <a href="#top">Ankit Kumar</a>
          <span>Code. Learn. Build. Repeat.</span>
          <a href="#top">Back to top ↑</a>
        </footer>
      </main>
      {isProjectRoute && <ProjectDetail route={currentRoute} project={projectForRoute(projects, currentRoute)} />}
    </>
  );
}
